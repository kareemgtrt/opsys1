...
setInterval(() => {
  http.get(`http://rust-devil.glitch.me/`);
}, 280000);
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@460379387955511296>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
\`أوامر الحماية\` :closed_lock_with_key:
\`${prefix}settings limit bans=settings limitsban\` : set a limit for bans
\`${prefix}settings limit kicks=settings limitskick\` : kick limit
\`${prefix}settings limit remove channels=settings limitsroleD\` : channel deleting limit
\`${prefix}settings limit make channels=settings limitsroleC\` : creating channels limit
\`${prefix}settings limit remove channels=settings limitstime\` channel deleting limit
\`${prefix}settings limit bans time=settings limitsti تحديد الوقت الذي من خلالة يتم التبنيد كـ مثال اذا شخص بند 5 في دقيقة البوت يبنده
\`${prefix}antibots on\` : Prevent bots from entering
\`${prefix}antibots off\` : Allow bots to enter
...
client.on("message", message => {
  if (message.content === "when is wipe?") {
    message.channel.send("**Every two weeks on Tuesday 16:00 Time of Amsterdam**");
    message.channel.sendFile("");
  }
});

client.on("message", message => {
  if (message.content === "when is the wipe?") {
    message.channel.send("**Every two weeks on Tuesday 16:00 Time of Amsterdam**");
    message.channel.sendFile("");
  }
});
...
var statuss = ["!help","Welcome to Rust Devil","Enjoy your stay!"]; // بالإمكان اضافة المزيد بدون التعديل في الكود
